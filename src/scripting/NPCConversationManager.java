...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void warpBack(int mid, final int retmap, final int time) { //時間秒數
        ...
        Timer.EventTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                if (c.getPlayer() != null) {
                    c.getSession().write(MaplePacketCreator.stopClock());
                    c.getPlayer().changeMap(warpMap, warpMap.getPortal(0));
                    c.getPlayer().dropMessage(5, "已經到達目的地了!");
                }
            }
    ...
    public static boolean hairExists(int hair) {
        return GameConstants.hairList.containsKey(hair);
    }
    ...
    public int[] getCanHair(int[] hairs) {
        ...
        if (cantHair.size() > 0 && c.getPlayer().isAdmin()) {
            StringBuilder sb = new StringBuilder("正在讀取的髮型裏有");
            sb.append(cantHair.size()).append("個髮型客戶端不支持顯示，已經被清除：");
    ...
    public static boolean faceExists(int face) {
        return GameConstants.faceList.containsKey(face);
    }
    ...
    public int[] getCanFace(int[] faces) {
        ...
        if (cantFace.size() > 0 && c.getPlayer().isAdmin()) {
            StringBuilder sb = new StringBuilder("正在讀取的臉型裏有");
            sb.append(cantFace.size()).append("個臉型客戶端不支持顯示，已經被清除：");
