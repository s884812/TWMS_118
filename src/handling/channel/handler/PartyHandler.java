            ...
            if (party != null) {
                if (action == 0x1B) { //accept
                    if (party.getMembers().size() < 6) {
                        ...
                        c.getSession().write(MaplePacketCreator.partyStatusMessage(0x12, null));
                    }
                } else if (action != 0x16) {
                    ...
                    if (cfrom != null) {
                        if (cfrom.getLevel() < 10) {
                            c.getPlayer().dropMessage(5, "對方等級低於10級，無法進行組隊。");
                        } else {
                            //cfrom.getClient().getSession().write(MaplePacketCreator.partyStatusMessage(23, c.getPlayer().getName()));
                            cfrom.dropMessage(5, "'" + c.getPlayer().getName() + "'拒絕組隊邀請。");
                        }
                    }
                }
            } else {
                c.getPlayer().dropMessage(5, "你要加入的隊伍不存在。");
            }
        } else {
                    ...
                    if (invited != null && invited.getParty() == null && invited.getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
                        ...
                        if (party.getMembers().size() < 6) {
                            if (invited.getLevel() < 10) {
                                ...
                                c.getSession().write(MaplePacketCreator.partyStatusMessage(0x16, invited.getName()));
                        ...
                        c.getPlayer().dropMessage(5, "對方已加入其他隊伍。");
                        //c.getSession().write(MaplePacketCreator.partyStatusMessage(0x11, null));
                    }
                } else {
                    c.getSession().write(MaplePacketCreator.partyStatusMessage(0x14, null));
                }
                break;
            case 5: // expel
