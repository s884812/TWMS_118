...
public class MapleQuest implements Serializable {
    ...
    public void start(MapleCharacter c, int npc) {
        if ((autoStart || checkNPCOnMap(c, npc)) && canStart(c, npc)) {
            ...
            /*if (c.getQuestStatus(getId()) == 0) { //任務未接取
    ...
    public void complete(MapleCharacter c, int npc, Integer selection) {
        if (c.getMap() != null && (autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)) {
            ...
            c.getClient().getSession().write(MaplePacketCreator.showSpecialEffect(0xA)); // Quest completion
    ...
    public static enum MedalQuest {
        新手冒險家(29005, 29015, 15, new int[]{100000000, 100020400, 100040000, 101000000, 101020300, 101040300, 102000000, 102020500, 102030400, 102040200, 103000000, 103020200, 103030400, 103040000, 104000000, 104020000, 106020100, 120000000, 120020400, 120030000}),
