...
public class MapScriptMethods {
    ...
    private static final String[] mulungEffects = {
        "我一直在等你！如果你有勇氣，你會從那道門進入！",
        "勇敢的你將踏上挑戰睦隆道場的路！",
        "我保證你將後悔挑戰睦隆道場！",
        "你有你的勇氣與毅力！但不要把你的勇氣與毅力當作魯莽行事的標準！",
        "如果你想一步一步地走到失敗的道路上，用自己的辦法去戰勝！"};
    ...
    public static void startScript_FirstUser(MapleClient c, String scriptName) {
        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            ...
            default: {
                ...
                FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "未處理的腳本 : " + scriptName + ", 型態 : onFirstUserEnter - 地圖ID " + c.getPlayer().getMapId());
                break;
            }
        }
    }
    ...
    public static void startScript_User(final MapleClient c, String scriptName) {
        ...
        if (c.getPlayer().isAdmin()) {
            System.out.println("[系統提示]您已經建立與地圖腳本: " + scriptName + " 地圖ID: " + c.getPlayer().getMapId() + " 的連接。(User)");
        }
        String data = "";
        ...
        if (c.getPlayer().isAdmin()) {
            c.getPlayer().dropMessage(6, "執行地圖腳本 onUserEnter：" + scriptName + " " + c.getPlayer().getMap().getMapName() + "(" + c.getPlayer().getMapId() + ")");
        }
    }
