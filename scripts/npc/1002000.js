...
function action(mode, type, selection) {
    ...
    if (status == 1)
        ...
        if (selection == 0) {
            ...
            var selStr = cm.getJob() == 0 || 1000 || 2000 || 2001 ? "你是新手所以你有90%的折扣\r\n請問你想去哪??#b" : "哦，你不是一個新手，是吧？我怕我可能會向您收取全價。你想去哪？#b";
            for (var i = 0; i < maps.length; i++)
                selStr += "\r\n#L" + i + "##m" + maps[i] + "# (" + (cost[i] * (cm.getJob() == 0 || 1000 || 2000 || 2001 ? 1 : 10)) + "楓幣)#l";
        ...
        selectedMap = selection;
        cm.sendYesNo("我猜你不想待這裡。你真的想要移動到 #b#m" + maps[selection] + "##k? 我將相你收取 #b" + (cost[selection] * (cm.getJob() == 0 || 1000 || 2000 || 2001 ? 1 : 10)) + " 楓幣#k. 你怎麼看??");
    } else if (status == 4) {
        if (cm.getMeso() < (cost[selectedMap] * (cm.getJob() == 0 || 1000 || 2000 || 2001 ? 1 : 10)))
            cm.sendNext("你沒有足夠的楓幣我不能幫助你!!");
        else {
            cm.gainMeso(-(cost[selectedMap] * (cm.getJob() == 0 || 1000 || 2000 || 2001 ? 1 : 10)));
