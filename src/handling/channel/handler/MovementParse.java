...
public class MovementParse {//1 = player, 2 = mob, 3 = pet, 4 = summon, 5 = dragon
    ...
    public static List<LifeMovementFragment> parseMovement(final LittleEndianAccessor lea, final int kind) {
        short posx = lea.readShort();
        short posy = lea.readShort();
        ...
        for (byte i = 0; i < numCommands; i++) {
            ...
            switch (command) {
                ...
                case 0x1D: {
                    ...
                    final RelativeLifeMovement rlm = new RelativeLifeMovement(command, new Point(posx, posy), duration, newstate);
                ...
                case 0x0E: {
                    ...
                    JumpDownMovement jump = new JumpDownMovement(command, new Point(posx, posy), duration, newstate);
                    jump.setFH(fh);
                ...
                case 0x15: {
                    byte newstate = lea.readByte();
                    short duration = lea.readShort();
                    GroundMovement ground = new GroundMovement(command, new Point(posx, posy), duration, newstate);
                    res.add(ground);
                }
                case 0x17: {
                    final short xpos = lea.readShort();
        ...
        if (numCommands != res.size()) {
            ...
            FileoutputUtil.log("logs/移動封包出錯.txt", "循環次數[" + numCommands + "]和實際上獲取的循環次數[" + res.size() + "]不符 " + "移動封包 剩餘次數: " + (numCommands - res.size()) + "  封包: " + lea.toString(true));
