...
//      final boolean fixed = ServerConstants.Use_Fixed_IV && !ServerConstants.isEligible(address);
            ...
            if (recv.getValue() == header_num) {
                ...
                try {// disable, i doesn't need this atm since log is added
                    ...
                    try {
                        ...
                        FileoutputUtil.log("logs/封包異常.txt", new StringBuilder().append("封包: ").append(lookupRecv(header_num)).append("\r\n").append(slea.toString(true)).toString());
                        FileoutputUtil.outputFileError("logs/封包異常.txt", ex);
                    }
    ...
    public static final void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c, final boolean cs) throws Exception {
        switch (header) {
            ...
            case CLIENT_ERROR:
                ...
                try {
                    try (RandomAccessFile file = new RandomAccessFile("logs/錯誤信息.txt", "rw")) {
