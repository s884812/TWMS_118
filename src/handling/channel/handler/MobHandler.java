...
public class MobHandler {
    ...
    public static final void MoveMonster(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (res != null && chr != null && res.size() > 0) {
            ...
            c.getSession().write(MobPacket.moveMonsterResponse(monster.getObjectId(), moveid, monster.getMp(), aggro, realskill, level));

            if (slea.available() != 16) {
                System.out.println("slea.available != 16 (怪物移動出錯) 剩餘封包長度: " + slea.available());
                FileoutputUtil.log("logs/怪物移動錯誤.txt", "slea.available != 16 (怪物移動錯誤)\r\n怪物ID: " + monster.getId() + "\r\n" + slea.toString(true));
                return;
            }
