...
public final class MapleMap {
    ...
    private void spawnAndAddRangedMapObject(final MapleMapObject mapobject, final DelayedPacketCreation packetbakery) {
        ...
        try {
            ...
            while (itr.hasNext()) {
                chr = itr.next();
                if (!chr.isClone() && (mapobject.getType() == MapleMapObjectType.MIST || (chr.getTruePosition().distance(mapobject.getTruePosition()) <= GameConstants.maxViewRangeSq())/*chr.getTruePosition().distanceSq(mapobject.getTruePosition()) <= GameConstants.maxViewRangeSq()*/)) {
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
    ...
    public final void updateMonsterController(final MapleMonster monster) {
        ...
        if (monster.getController() != null) {
            if (monster.getController().getMap() != this || monster.getController().getTruePosition().distance(monster.getTruePosition()) > monster.getRange()/*monster.getController().getTruePosition().distanceSq(monster.getTruePosition()) > monster.getRange()*/) {
        ...
        try {
            for (MapleCharacter chr : characters) {
                if (!chr.isHidden() && !chr.isClone() && (chr.getControlledSize() < mincontrolled || mincontrolled == -1) && (chr.getTruePosition().distanceSq(monster.getTruePosition()) <= monster.getRange())/*chr.getTruePosition().distanceSq(monster.getTruePosition()) <= monster.getRange()*/) {
    ...
    public final void spawnMobDrop(final Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, final byte droptype, final int questid) {
        ...
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            ...
            @Override
            public void sendPackets(MapleClient c) {
                if (c != null && c.getPlayer() != null && (questid <= 0 || c.getPlayer().getQuestStatus(questid) == 1) && (idrop.getItemId() / 10000 != 238 || c.getPlayer().getMonsterBook().getLevelByCard(idrop.getItemId()) >= 0) && mob != null && dropPos != null) {
...
//	broadcastMessage(MaplePacketCreator.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (hasBoat() == 2 && (chr.getMapId() != 200090000 || chr.getMapId() != 200090010)) {
            ...
            chr.getClient().getSession().write(MaplePacketCreator.boatPacket(2));
        }
        if (getBoatEffect() && (chr.getMapId() == 200090000 || chr.getMapId() == 200090010)) {
            chr.getClient().getSession().write(MaplePacketCreator.boatEffect(1034));
            chr.getClient().getSession().write(MaplePacketCreator.musicChange("Bgm04/ArabPirate"));
        }
    }
            ...
            for (MapleCharacter chr : characters) {
                if (chr != source) {
                    if (rangeSq < Double.POSITIVE_INFINITY) {
                        if (/*rangedFrom.distanceSq(chr.getTruePosition()) <= rangeSq*/rangedFrom.distance(chr.getTruePosition()) <= rangeSq) {
                            chr.getClient().getSession().write(packet);
                        }
                    } else {
                        chr.getClient().getSession().write(packet);
    ...
    public final void updateMapObjectVisibility(final MapleCharacter chr, final MapleMapObject mo) {
        ...
        if (!chr.isMapObjectVisible(mo)) { // monster entered view range
            if (mo.getType() == MapleMapObjectType.MIST || mo.getType() == MapleMapObjectType.EXTRACTOR || mo.getType() == MapleMapObjectType.SUMMON || mo.getType() == MapleMapObjectType.FAMILIAR || mo instanceof MechDoor || (mo.getTruePosition().distance(chr.getTruePosition()) <= mo.getRange())/*(mo.getTruePosition().distanceSq(chr.getTruePosition()) <= mo.getRange())*/) {
                chr.addVisibleMapObject(mo);
                mo.sendSpawnData(chr.getClient());
            }
        } else if (!(mo instanceof MechDoor) && mo.getType() != MapleMapObjectType.MIST && mo.getType() != MapleMapObjectType.EXTRACTOR && mo.getType() != MapleMapObjectType.SUMMON && mo.getType() != MapleMapObjectType.FAMILIAR && (mo.getTruePosition().distance(chr.getTruePosition()) > mo.getRange())/*(mo.getTruePosition().distanceSq(chr.getTruePosition()) > mo.getRange())*/) {
            chr.removeVisibleMapObject(mo);
            mo.sendDestroyData(chr.getClient());
        } else if (mo.getType() == MapleMapObjectType.MONSTER) { //monster didn't leave view range, and is visible
            if (chr.getTruePosition().distance(mo.getTruePosition()) <= GameConstants.maxViewRangeSq_Half()/*chr.getTruePosition().distanceSq(mo.getTruePosition()) <= GameConstants.maxViewRangeSq_Half()*/) {
    ...
    private boolean boat, docked, boatEffect = false;
