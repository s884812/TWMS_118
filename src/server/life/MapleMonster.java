...
//              exp = ChannelServer.getInstance(map.getChannel()).getExpRate() / 5; // depend on your rate and /
    ...
    public final void sendSpawnData(final MapleClient client) {
        ...
        sendStatus(client);
        if (map != null && !stats.isEscort() && client.getPlayer() != null && (client.getPlayer().getTruePosition().distance(getTruePosition()) <= GameConstants.maxViewRangeSq_Half())/*(client.getPlayer().getTruePosition().distanceSq(getTruePosition()) <= GameConstants.maxViewRangeSq_Half())*/) {
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (from.isAdmin() && LoginServer.isLogPackets()) {
            from.dropMessage(6, " 怪物: " + getId() + " 怪物: " + status.getStati().name() + " 中毒: " + poison + " 持續時間: " + duration);
        }
