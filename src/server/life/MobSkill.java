    ...
    public void applyEffect(MapleCharacter player, MapleMonster monster, boolean skill) {
        ...
        switch (skillId) {
            ...
            case 143: // Weapon Reflect
                ...
                stats.put(MonsterStatus.WEAPON_IMMUNITY, Integer.valueOf(x));
                reflection.add(x);
                monster.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, "[系統提示] 注意 " + monster.getStats().getName() + " 開啟了反射物理傷害状态。"));
                break;
            case 144: // Magic Reflect
                ...
                stats.put(MonsterStatus.MAGIC_IMMUNITY, Integer.valueOf(x));
                reflection.add(x);
                monster.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, "[系統提示] 注意 " + monster.getStats().getName() + " 開啟了反射魔法傷害状态。"));
                break;
            case 145: // Weapon / Magic reflect
                ...
                reflection.add(x);
                reflection.add(x);
                monster.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, "[系統提示] 注意 " + monster.getStats().getName() + " 開啟了反射物理傷害和魔法傷害状态。"));
                break;
            case 200:
                if (monster == null) {
                    return;
