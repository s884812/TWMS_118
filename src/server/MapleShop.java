...
public class MapleShop {
    ...
    public void sendShop(MapleClient c) {
        ...
        if (c.getPlayer().isAdmin()) {
            if (npc == null || npc.getName().equals("MISSINGNO")) {
                c.getPlayer().dropMessage(1, "商店" + id + "找不到此代碼為" + getNpcId() + "的Npc。");
                return;
            } else if (c.getPlayer().isAdmin()) {
                c.getPlayer().dropMessage(6, "您已建立與商店" + id + "的連接。");
            }
        }
    ...
    public void buy(MapleClient c, int itemId, short quantity) {
        ...
        if (itemId / 10000 == 190 && !GameConstants.isMountItemAvailable(itemId, c.getPlayer().getJob())) {
            c.getPlayer().dropMessage(1, "你不可以購買這個道具。");
            ...
            if (price >= 0 && c.getPlayer().getMeso() >= price) {
                if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                    ...
                    if (GameConstants.isPet(itemId)) {
                        MapleInventoryManipulator.addById(c, itemId, quantity, "", MaplePet.createPet(itemId, MapleInventoryIdentifier.getInstance()), -1, "購買NPC商店物品: " + id + ", " + npcId + " 時間 " + FileoutputUtil.CurrentReadable_Date());
                    } else {
                        if (GameConstants.isRechargable(itemId)) {
                            quantity = ii.getSlotMax(item.getItemId());
                        }

                        MapleInventoryManipulator.addById(c, itemId, quantity, "購買NPC商店物品: " + id + ", " + npcId + "時間 " + FileoutputUtil.CurrentReadable_Date());
                    }
                } else {
                    c.getPlayer().dropMessage(1, "您的背包是滿的，請整理下背包。");
                }
                c.getSession().write(MaplePacketCreator.confirmShopTransaction((byte) 0, this, c, -1));
            }
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            while (rs.next()) {
                ...
                if (GameConstants.isThrowingStar(rs.getInt("itemid")) || GameConstants.isBullet(rs.getInt("itemid"))) {
                    MapleShopItem starItem = new MapleShopItem((short) 1, rs.getInt("itemid"), rs.getInt("price"), rs.getInt("reqitem"), rs.getInt("reqitemq"), rs.getByte("rank"), rs.getInt("category"));
                    ret.addItem(starItem);
                    if (rechargeableItems.contains(starItem.getItemId())) {
                        recharges.remove(Integer.valueOf(starItem.getItemId()));
                    }
                } else {
                    ret.addItem(new MapleShopItem((short) 1000, rs.getInt("itemid"), rs.getInt("price"), rs.getInt("reqitem"), rs.getInt("reqitemq"), rs.getByte("rank"), rs.getInt("category")));
                }
            }
