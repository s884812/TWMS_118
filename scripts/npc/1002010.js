...
function start() {
    action(1, 0, 0);
    if (cm.isGMS()) {
        maps = Array(802000101, 101000000, 105000000, 910001000, 680000000, 230000000, 260000000, 800000000, 211000000, 120030000, 130000200, 100000000, 103000000, 222000000, 240000000, 240070000, 104000000, 220000000, 120000000, 221000000, 200000000, 102000000, 300000000, 801000000, 540000000, 541000000, 250000000, 251000000
        ...
        pqMaps = Array(310000000, 610030010, 931000310, 931000311, 931000312, 541010040, 541010050, 801040004, 240040000, 211060100, 211060300, 211060500, 211060700, 211060900, 551030100, 211042400, 211042401, 240050400, 921140001, 270050200, 271040000);
    } else {
        maps = Array(802000101, 101000000, 105000000, 910001000, 680000000, 230000000, 260000000, 800000000, 211000000, 120030000, 130000200, 100000000, 103000000, 222000000, 240000000, 240070000, 104000000, 220000000, 120000000, 221000000, 200000000, 102000000, 300000000, 801000000, 540000000, 541000000, 250000000, 251000000
            , 551000000, 550000000, 800040000, 261000000, 541020000, 270000000, 682000000, 140000000, 970010000, 103040000, 555000000, 310000000, 200100000, 211060000, 310040300, 970020000, 960000000, 600000000); 
        pqMaps = Array(610030010,931000310, 931000311, 931000312, 541010040, 541010050, 801040004, 240040000, 211060100, 211060300, 211060500, 211060700, 211060900, 551030100, 211042400, 211042401, 240050400, 921140001, 270050200, 271040000);
    }
}
...
function action(mode, type, selection) {
	...
	cm.gainItem(2000005, 200);
                ...
                cm.sendOk("Welcome to AcrasiaMS! I congratulate you on getting to level 10. There is still a long way to go, Here are some items to help you on your journey with us!");
            } else {
        ...
        cm.sendSimple("Hello Agent #r#h ##k. \r\n#b#b#L9990#Explorer Job Advance#k#l\r\n#L9991#Legends/Blade Master Job Advancer#k#l\r\n#L9992#Knights Of Cygnus Job Advancer#k#l\r\n#L9993#Valkyrie and Buccaneer 4th Job Advancer.#k#l\r\n");
    } else if (status == 1) {
        if (selection == 12) {
            cm.dispose();
            cm.openNpc(9201117);
        ...
        if (selection == 1) {
            ...
            if (cm.getPlayer().getSkillLevel(80001000) > 0 || cm.getPlayer().getSkillLevel(cm.getPlayer().getStat().getSkillByJob(1004, cm.getPlayer().getJob()))) {
                ...
                cm.teachSkill(cm.isGMS() ? 80001000 : cm.getPlayer().getStat().getSkillByJob(1016, cm.getPlayer().getJob()), 1, 0); // Maker
